...
statusPorcentajeCotas: String;
  ...
  constructor(
    ...
    this.tiposOperacion = this.tiposOperacion.map((tiposOp) =>
      tiposOp.filter(
        (tipoOp) =>
          tipoOp.idSisTipoOperacion == 4 || tipoOp.idSisTipoOperacion == 10
      )
    );
  ...
  onClickEdit = (tipoColumnas) => (itemSelect: any) => {
    ...
    this.tablas.columnas[tipoColumnas] = this.tablas.columnas[tipoColumnas].map(
      (tabla) => {
        let newTabla = tabla;
  ...
  buscaEstadosSisa = (idPadron) => {
    this.emisionRemitosService.getEstadosSisa(idPadron).subscribe((data) => {
      ...
      : this.utilsService.showModal("Confirmar")(
          ...
          if (this.fechaComprobanteInvalida()) {
            ...
            this.emisionRemitosService
              ...
              )(this.sisCanje)(this.formasPagoSeleccionadas)(this.factura)(
                this.tablas.datos.modelosFactura
              )(this.tablas.datos.detallesFormaPago)(this.deposito)(
                this.tablas.datos.lotesTraza
              )(this.tipoOperacion)(this.dataVendedor)(
                this.tablas.datos.subtotalesProductos
              )(this.listaPrecioSelect)(this.contrato)(this.relacionCanje)(
                this.cereal
              )(this.diferidoVto)(this.pesificado)(this.marcaPesificado)(
          ...
          this.emisionRemitosService
            ...
            .then((vendedorAsociado) => {
              ...
              if (vendedorAsociado) {
                ...
                this.emisionRemitosService.llenarPadron(this.cliente);
              }
  ...
  keyPressInputTexto = (e: any) => (upOrDown) => {
    ...
    this.clienteEnfocadoIndex = this.popupListaService.keyPressInputForPopup(
      upOrDown
    )(this.clientes.filtrados.value)(this.clienteEnfocadoIndex);
  };
      ...
      .map((fp) =>
        Object.assign([], fp.detalles).map((det) => {
          ...
          auxDet.fechaPago = new DateLikePicker(
            moment(
              this.utilsService.dateLikePickerToDate(this.comprobante.fechaVto)
            )
    ...
    if (options.fromBuscaPendiente && prod.importe === 0) {
      ...
      return this.emisionRemitosService
        ...
        .then((nuevoSubtotal) => {
          ...
          const viejoSubtotal = this.tablas.datos.subtotalesProductos.find(
  ...
  actualizarTotalNeto = () => {
    ...
    this.cotizacionDatos.total = this.comprobante.tipo.comprobante.incluyeNeto
      ? _.sumBy(this.tablas.datos.productosPend, (prod) => {
          // Busco el subtotal
          const subtotalBuscado = this.tablas.datos.subtotalesProductos.find(
   ...
   */
  calcRestoPagar = () => {
    const sumMontos = _.sumBy(this.tablas.datos.detallesFormaPago, (fPago) =>
      Number(fPago.monto) ? Number(fPago.monto) : 0
    );
  ...
  onBlurNumeradorCteRel = (e) =>
    this.utilsService.onBlurInputNumber(e) && this.comprobanteRelacionado.numero
  ...
  onClickConfirmRelacionado = () => {
    if (this.numeroComprobanteAfipRelacionado && this.ptoVentaAfipRelacionado) {
  ...
  onClickBuscarPesificado = () => {
    this.emisionRemitosService
      ...
      .subscribe((response) => {
        if (response) {
          this.utilsService.showModal("OK")("Comprobante encontrado")()();
          ...
          if (response.cotDolar < this.cotizacionDatos.cotizacion.cotizacion) {
            this.comprobante.tipo =
            ...
            this.monedas = this.comprobante.tipo.comprobante.monedas;
            this.comprobante.moneda =
              this.monedas[
                this.monedas.findIndex((value) => value.idMoneda === 1)
              ];
            ...
            if (response.diferidoVto) {
              ...
              valorTotal105 = valorTotal105 + valorTotal105 * 0.105;
            }
            ...
            response.cotDolar > this.cotizacionDatos.cotizacion.cotizacion
          ) {
            this.comprobante.tipo =
              this.tiposComprobantes[
                this.tiposComprobantes.findIndex(
